...
#   2. Exec the TS worker that does provisioning, server boot, helper spawn,
#      file write, vault_files / Storage assertions, delete, 401 sign-out,
#      and full cleanup.
#
...
# SwiftPM nests builds under .build/<arch>/debug/ as well as exposing a
# stable .build/debug/ symlink. Prefer the symlink; fall back to arch-suffixed
# path for older toolchains where the symlink is absent.
...
# ─── Exec the TS worker ────────────────────────────────────────────────────
# cd to repo root so the worker's inner `spawn("npx", ["tsx", "src/index.ts",
# ...])` resolves relative paths regardless of how this script was invoked.
cd "${REPO_ROOT}"
log "Running scripts/smoke-sync-push.ts…"
exec npx tsx "${SCRIPT_DIR}/smoke-sync-push.ts"
